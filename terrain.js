...
// PolyU 2019: lines[0] is the police cordon (steel). Label + colour come from
// the data (name_zh/name_en), not a hardcoded 1941 line. (Engine diff: this is
// content, not mechanics — the tube geometry/animation is unchanged.)
const CORDON_COL = 0x86a9cf;   // FAC.uk.glow (steel)
let gdlMesh=null; export let gdlLabel=null;
export function buildLine(){
  const ln=D.geography.lines[0];
  const curve=geoCurve(ln.path, 12);
  gdlMesh=new THREE.Mesh(new THREE.TubeGeometry(curve,120,2.0,8,false),
    new THREE.MeshBasicMaterial({color:CORDON_COL, transparent:true, opacity:0}));
  scene.add(gdlMesh);
  gdlLabel=lineLabel(ln.name_zh||"封鎖線", ln.name_en||"Cordon", "#86a9cf");
  const m=curve.getPoint(0.5); gdlLabel.o.position.set(m.x, m.y+CFG.LBL_FORT, m.z);
}
// The cordon forms as police seal the campus on 17 Nov; fade it in then hold it.
export function updateLines(day){
  if(!gdlMesh) return;
  const op = clamp((day-16)/1.0,0,1)*0.55;
