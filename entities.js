...
export function buildUnit(u){
  ...
  // PolyU 2019 · de-militarised: no national flags. flagTexture() returns null,
  // so the pole / finial / cloth are skipped entirely and a unit reads as a
  // neutral side-coloured marker + name. (Built only if a texture exists, so the
  // engine still works unchanged if flags were ever re-enabled.)
  const tex=flagTexture(u);
  let pole=null, finial=null, flag=null;
  if(tex){
    pole=new THREE.Mesh(new THREE.CylinderGeometry(CFG.POLE_R,CFG.POLE_R,CFG.FLAG_H+CFG.TOKEN_H,6),
      new THREE.MeshStandardMaterial({color:0x2a2620, roughness:0.8}));
    pole.position.y=(CFG.FLAG_H+CFG.TOKEN_H)/2; pole.castShadow=true; grp.add(pole);
    finial=new THREE.Mesh(new THREE.SphereGeometry(CFG.FINIAL_R,8,8),
    ...
    flag=new THREE.Mesh(fg, new THREE.MeshStandardMaterial({map:tex,
  ...
  // no pole → seat the name label just above the ground marker (not at flag height)
  const lbl=new THREE.CSS2DObject(div); lbl.position.set(0, (tex?CFG.FLAG_H:CFG.TOKEN_H)+CFG.LBL_UNIT, 0); grp.add(lbl);
  unitsGroup.add(grp);
...
export function updateUnits(day){
  ...
  unitObjs.forEach(o=>{
    ...
    // flags are disabled (o.flag/o.finial are null) → guard every flag access.
    if(o.flag){ o.flag.visible=true; o.flag.material.opacity=focused?1:0.5; }
    if(o.finial) o.finial.visible=focused;
    ...
    if(dead){ o.token.material.color.setHex(0x55585c); o.token.material.emissiveIntensity=focused?0.06:0.0;
      o.ring.material.opacity=focused?0.4:DIM; if(o.flag){ o.flag.rotation.z=-0.5; o.flag.position.y=(CFG.FLAG_H+CFG.TOKEN_H)*0.55; } }
    else { o.token.material.color.setHex(f.main); if(o.flag){ o.flag.rotation.z=0; o.flag.position.y=CFG.FLAG_H+CFG.TOKEN_H-CFG.FLAG_TH*0.6; }
