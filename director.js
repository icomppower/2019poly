...
/* ===================== CAMERA & DIRECTOR ========================= */
const TITLE_DUR=4.5;
// closing shot: a slow pull-back/orbit over the campus + Hung Hom (campus-scale, not HK-wide)
const OUTRO_CAM={ lng:114.180, lat:22.3035, dist:1500, az:0, el:50, orbit:0.8, tween:3.6 };
...
export const Director = {
  ...
  start(){ this.mode="title"; this.t=0; this.playing=true; this.userFree=false; setDay(this.shots[0].day);
    ...
    const I=D.intro||{}; card(I.title_zh||"理大圍城 2019", I.title_en||"THE SIEGE OF POLYU · 2019", I.narration_zh||"", I.narration_en||""); },
...
function updateProgress(){ const N=Director.shots.length; let f=0;
  ...
  $("scene-label").textContent = Director.mode==="outro" ? "完 · END"
    : (Director.mode==="play" ? `2019.11.${String(clamp(Math.floor(Clock.day),11,30)).padStart(2,"0")}` : ""); }
...
export function wireUI(){
  ...
  // PolyU 2019: background music is omitted (no period-appropriate neutral track),
  // so #bgm / #music-btn are absent. Everything below is guarded so the UI and the
  // play/resume wiring still work; syncMusic() becomes a no-op when there is no audio.
  const bgm=$("bgm"), musicBtn=$("music-btn");
  if(bgm){ bgm.volume=0.55; bgm.muted=true; }
  let soundOn=false;
  const paintMusic=()=>{ if(musicBtn){ musicBtn.textContent=soundOn?"🔊":"🔇"; musicBtn.classList.toggle("off",!soundOn); } };
  const syncMusic=()=>{ if(!bgm) return; if(Director.playing){ bgm.play().catch(()=>{}); } else { bgm.pause(); } };
  paintMusic();
  if(musicBtn) musicBtn.onclick=()=>{ soundOn=!soundOn; if(bgm) bgm.muted=!soundOn; syncMusic(); paintMusic(); };
